...
"📜 <b>Check the land records yourself:</b> exact plot corners live on <b>biharbhumi.bihar.gov.in</b> — open it in your own browser → Jamabandi/Khasra → Patna → Punpun block → mauja <b>Pothahi</b> (also check <b>Dharahara</b>), and look for the ~38.77-acre Agriculture Department plot transferred to the Law Department (Sep 2025 cabinet). The portal asks for a security code, so only you can run this lookup.<br>" +
  ...
  /* ---------- Overpass loading (compacted 24 h cache) ----------
     Only the fields renderElement actually reads are cached, and
     coordinates are rounded to ~1 m. This keeps a ~28k-feature
     payload well under the ~5 MB localStorage limit. */
  const CACHE_TAGS = ["highway", "shop", "building", "landuse", "leisure", "amenity", "name", "name:en", "addr:full"];

  function compactElement(e) {
    const t = {}, src = e.tags || {};
    for (const k of CACHE_TAGS) if (src[k] !== undefined) t[k] = src[k];
    const out = { t: e.type, i: e.id, g: t };
    if (e.type === "node") { out.a = Math.round(e.lat * 1e5); out.o = Math.round(e.lon * 1e5); }
    else if (e.geometry) {
      // Delta-encoded integer coords (1e-5 deg ≈ 1 m). A raw lat/lon pair is
      // ~20 chars; a delta is typically 2–6 chars — this is what makes a
      // ~28k-feature payload fit under the ~5 MB localStorage limit.
      let pla = 0, plo = 0, first = true;
      out.gm = e.geometry.map(p => {
        const la = Math.round(p.lat * 1e5), lo = Math.round(p.lon * 1e5);
        const v = first ? [la, lo] : [la - pla, lo - plo];
        first = false; pla = la; plo = lo;
        return v;
      });
    }
    return out;
  }
  function expandElement(c) {
    const e = { type: c.t, id: c.i, tags: c.g };
    if (c.t === "node") { e.lat = c.a / 1e5; e.lon = c.o / 1e5; }
    else if (c.gm) {
      let la = 0, lo = 0, first = true;
      e.geometry = c.gm.map(d => {
        if (first) { la = d[0]; lo = d[1]; first = false; }
        else { la += d[0]; lo += d[1]; }
        return { lat: la / 1e5, lon: lo / 1e5 };
      });
    }
    return e;
  }
  ...
  function readCache() {
    try {
      ...
      if (!Array.isArray(c.el) || !c.el.length) return null; // rejects stale old-format caches
      return { elements: c.el.map(expandElement) };
    } catch (e) { return null; }
  }
  /* Returns true ONLY when a usable cache actually persisted. */
  function writeCache(data) {
    try {
      const payload = JSON.stringify({ ts: Date.now(), el: (data.elements || []).map(compactElement) });
      localStorage.setItem(cfg.cacheKey, payload);
      return localStorage.getItem(cfg.cacheKey) === payload; // quota/private-mode can fail silently
    } catch (e) {
  ...
  /* Live OSM layer keys (the ones fetched from Overpass). Project layers
     and user listings are restored independently and never depend on this. */
  const LIVE_KEYS = ["roads", "malls", "apartments", "openland", "amenities", "shops"];

  async function loadData(force, showOverlay) {
    if (showOverlay) el("loading").classList.remove("hidden");
    setLiveBadge("loading");
    // NOTE: live layers are cleared only AFTER a successful fetch.
    // A failed reload must leave the map exactly as it was, never blank.
    try {
      let data = null, fromCache = false, saved = false;
      const cached = readCache();

      if (!force && cached) { data = cached; fromCache = true; }
      else {
        const q = buildQuery();
        ...
        saved = writeCache(data);
        LIVE_KEYS.forEach(k => { LAYERS[k].group.clearLayers(); counts[k] = 0; });
        roadKm = 0;
      }

      const n = (data.elements || []).length;
      (data.elements || []).forEach(renderElement);
      const detail = n + " features" + (saved ? " · ✓ saved" : "");
      setLiveBadge(fromCache ? "cached" : "ok", detail);
      if (force) toast("Loaded live data: " + n + " features.", false);
    } catch (e) {
      console.error(e);
      const liveDrawn = LIVE_KEYS.some(k => counts[k] > 0);
      if (liveDrawn) {
        // Priority: keep what's already on the map. Don't wipe, don't fake a cache.
        setLiveBadge("error");
        toast("Live refresh failed — keeping the map data already shown.", true);
      } else {
        const cached = readCache();
        if (cached && (cached.elements || []).length) {
          (cached.elements || []).forEach(renderElement);
          setLiveBadge("cached");
          toast("Offline — showing saved map data.", true);
        } else {
          setLiveBadge("error");
